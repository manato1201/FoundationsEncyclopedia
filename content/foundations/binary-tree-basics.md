...
operationSteps:
  - label: ルートノードの値と比較する
    note: 探している値がルートより小さいか大きいかを確認する
  - label: 左か右の子へ進む
    note: 小さければ左の子、大きければ右の子へ移動する
  - label: 到達するまで繰り返す
    note: 子ノードがなくなるか、値が一致するまで同じ比較を続ける
  - label: 見つかった位置で検索・挿入を行う
    note: この経路がO(log n)の計算量の由来になる(木がバランスしている場合)
---
