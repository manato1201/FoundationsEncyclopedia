...
operationSteps:
  - label: 攻撃者が悪意あるスクリプトを入力する
    note: "掲示板の投稿欄等に <script>...</script> を含む文字列を送信する"
  - label: サーバーがエスケープせずに保存・表示する
    note: 脆弱性があると、入力値がそのままHTMLとして埋め込まれてしまう
  - label: 被害者のブラウザがスクリプトを実行する
    note: ページを開いた別のユーザーのブラウザ上で、攻撃者のスクリプトが実行される
  - label: Cookie窃取等の被害が発生する
    note: セッション情報の窃取や、意図しない操作の実行につながる
---
